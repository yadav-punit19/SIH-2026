...
import { Bell, Camera, LogOut, Radio, UserCheck } from "lucide-react";
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-panel/90 backdrop-blur">
      <div className="flex h-14 items-center justify-between gap-4 px-4 lg:px-6">
        ...
        <div className="flex items-center gap-3">
          {/* Active Operator Badge Info */}
          {operator && (
            <div className="hidden items-center gap-2 rounded-lg border border-border bg-panel-elevated px-3 py-1 text-xs sm:flex">
              <span className="grid size-6 place-items-center rounded-md bg-primary/20 text-primary">
                <UserCheck className="size-3.5" />
              </span>
              <div className="flex flex-col text-left">
                <span className="font-mono text-[10px] font-bold text-foreground">{operator.badge}</span>
